<template>
  <div class="page">
    <div class="add-people-box">
      <div class="box-title"><span></span>添加分阅人员</div>
...
import { getSeparate } from '@/api/circulate'
...
function btnSubmit() {
  if(!userList.arr.length) {
    showToast('请先选择要分阅的人员');
    return 
  }
  const userId = userList.arr.map((item: { id: any }) => item.id)
  const postObj = {
  ...
  getSeparate(postObj).then(res => {
    if (res.code === '0') {
      showSuccessToast('分阅成功！');
      router.back()
    }
  })
}
